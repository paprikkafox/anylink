      ...
      <el-col :span="6" class="card-panel-col">
        <div class="card-panel" v-on:click="jump('/admin/user/online')">
          ...
          <div class="card-panel-description">
            <div class="card-panel-text">Number of online</div>
...
export default {
  ...
  methods: {
    ...
    formatOnline(data) {
        ...
        let xnum = 0     
        chartData.xname = []
        chartData.xdata["Number of people online"] = []
        for(var i=0; i<datas.length;i++){
            ...
            chartData.xdata["Number of people online"][i] = xnum
        }
        // Real-time update of online numbers
    ...
    formatNetwork(data) {
        ...
        chartData.xname = []
        chartData.xdata["Upstream traffic"] = []
        chartData.xdata["Downstream traffic"] = []
        for(var i=0; i<datas.length;i++){
            chartData.xname[i] = this.dateFormat(datas[i].created_at, timeFormat)
            xnumUp = datas[i].up
            ...
            chartData.xdata["Upstream traffic"][i] = this.toMbps(xnumUp)
            chartData.xdata["Downstream traffic"][i] = this.toMbps(xnumDown)
        }
        this.lineChart[data.action] = chartData
    },
    formatCpu(data) {
