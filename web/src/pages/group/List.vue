          ...
          <template slot-scope="scope">
            ...
            <div v-if="scope.row.route_include.length > readMinRows">
              ...
              <el-button size="mini" type="text" @click="toggleMore(`ri_${ scope.row.id }`)">
                {{ readMore[`ri_${scope.row.id}`] ? "▲ Close" : "▼ Open" }}
              </el-button>
            </div>
          </template>
        </el-table-column>
          ...
          <template slot-scope="scope">
            ...
            <div v-if="scope.row.route_exclude.length > readMinRows">
              ...
              <el-button size="mini" type="text" @click="toggleMore(`re_${ scope.row.id }`)">
                {{ readMore[`re_${scope.row.id}`] ? "▲ Close" : "▼ Open" }}
              </el-button>
            </div>
          </template>
        </el-table-column>
          ...
          <template slot-scope="scope">
            ...
            <div v-if="scope.row.link_acl.length > readMinRows">
              ...
              <el-button size="mini" type="text" @click="toggleMore(`la_${ scope.row.id }`)">
                {{ readMore[`la_${scope.row.id}`] ? "▲ Close" : "▼ Open" }}
              </el-button>
            </div>
          </template>
        </el-table-column>
      ...
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="ruleForm">
        <el-tabs v-model="activeTab" :before-leave="beforeTabLeave">
          <el-tab-pane label="General" name="general">
            ...
            <el-form-item label="Bandwidth Limit" prop="bandwidth_format" style="width:260px;">
            ...
            <el-form-item label="Allow LAN" prop="allow_lan">
              <!-- active-text="After turning it on, the user's local network segment will not be encrypted via anylink" -->
              <el-switch v-model="ruleForm.allow_lan"></el-switch>
              <div class="msg-info">
                Note: Local network refers to:
                The network where the PC running the anyconnect client is located, that is, the local routing segment. After it is turned on, the data in the local routing segment of the PC will not be forwarded through the tunnel link. At the same time, the anyconnect client needs to check the switch of the local network (Allow Local Lan) for the function to take effect.
              </div>
            </el-form-item>

            <el-form-item label="Client DNS" prop="client_dns">
              <el-row class="msg-info">
                <el-col :span="20">Enter the IP format as follows: 192.168.0.10</el-col>
                <el-col :span="4">
            ...
            <el-form-item label="Split DNS" prop="split_dns">
              <el-row class="msg-info">
          ...
          <el-tab-pane label="Authorization type" name="authtype">
            <el-form-item label="Authorization type" prop="authtype">
            ...
            <template v-if="ruleForm.auth.type == 'radius'">
              <el-form-item label="Server address" prop="auth.radius.addr"
                ...
                <el-input v-model="ruleForm.auth.radius.addr" placeholder="For example, 192.168.20.123:1812"></el-input>
              </el-form-item>
              <el-form-item label="Server secret" prop="auth.radius.secret"
            ...
            <template v-if="ruleForm.auth.type == 'ldap'">
              <el-form-item label="Server address" prop="auth.ldap.addr"
                ...
                <el-input v-model="ruleForm.auth.ldap.addr" placeholder="For example -  192.168.20.123:389 / example.com:389"></el-input>
              </el-form-item>
              <el-form-item label="Use TLS" prop="auth.ldap.tls">
                <el-switch v-model="ruleForm.auth.ldap.tls"></el-switch>
              </el-form-item>
              <el-form-item label="Bind DN" prop="auth.ldap.bind_name"
              ...
              <el-form-item label="Bind PW" prop="auth.ldap.bind_pwd"
              ...
              <el-form-item label="Object class" prop="auth.ldap.object_class"
              ...
              <el-form-item label="Search attribute" prop="auth.ldap.search_attr"
              ...
              <el-form-item label="Group CN" prop="auth.ldap.member_of">
                <el-input v-model="ruleForm.auth.ldap.member_of"
                          placeholder="Optional, only allow specified groups to log in, for example - CN=HomeWork,DC=abc,DC=com"></el-input>
              </el-form-item>
            </template>
          </el-tab-pane>
          ...
          <el-tab-pane label="Routing settings" name="route">
            <el-form-item label="Include" prop="route_include">
              <el-row class="msg-info">
                <el-col :span="18">IP in CIDR format: 192.168.1.0/24</el-col>
                <el-col :span="2">
                  <el-button size="mini" type="success" icon="el-icon-plus" circle
            ...
            <el-form-item label="Exclude" prop="route_exclude">
              <el-row class="msg-info">
                <el-col :span="18">IP in CIDR format: 192.168.2.0/24</el-col>
                <el-col :span="2">
                  <el-button size="mini" type="success" icon="el-icon-plus" circle
          ...
          <el-tab-pane label="Domain Split Tunneling" name="ds_domains">
            <el-form-item label="Include" prop="ds_include_domains">
              <el-input type="textarea" :rows="5" v-model="ruleForm.ds_include_domains"
                        placeholder="Enter the domain name separated by ',' and by default it matches all subdomains, such as example.com"></el-input>
            </el-form-item>
      ...
      <el-form ref="ipEditForm" label-width="80px">
        <el-form-item label="Routing Table" prop="ip_list">
          <el-input type="textarea" :rows="10" v-model="ipEditForm.ip_list"
                    placeholder="One route per line, for example: 192.168.1.0/24"></el-input>
          <div class="msg-info">Total
        ...
        <el-form-item>
          <el-button type="primary" @click="ipEdit()" :loading="ipEditLoading">Save</el-button>
    ...
    return {
      ...
      rules: {
        name: [
          ...
          {max: 30, message: 'Group name must be less than 30 characters in length', trigger: 'blur'}
        ],
        bandwidth_format: [
          {required: true, message: 'Please enter the bandwidth limit', trigger: 'blur'},
        ...
        "auth.radius.addr": [
          {required: true, message: 'Please enter the Radius server', trigger: 'blur'}
        ],
        "auth.radius.secret": [
          {required: true, message: 'Please enter the Radius secret', trigger: 'blur'}
        ],
        "auth.ldap.addr": [
          {required: true, message: 'Please enter the LDAP server address (including port)', trigger: 'blur'}
        ],
        "auth.ldap.bind_name": [
          {required: true, message: 'Please enter the Bind DN', trigger: 'blur'}
        ],
        "auth.ldap.bind_pwd": [
          {required: true, message: 'Please enter the Bind PW', trigger: 'blur'}
        ],
        "auth.ldap.base_dn": [
          {required: true, message: 'Please enter the Base DN', trigger: 'blur'}
        ],
        "auth.ldap.object_class": [
          {required: true, message: 'Please enter the user object class', trigger: 'blur'}
        ],
        "auth.ldap.search_attr": [
          {required: true, message: 'Please enter your unique user ID', trigger: 'blur'}
        ],
      },
    }
  },
  methods: {
    ...
    ipEdit() {
      ...
      for (let i = 0; i < ipList.length; i++) {
        ...
        if (!valid.valid) {
          this.$message.error("Error: CIDR format is incorrect, it is recommended to change" + ip[0] + "" + valid.suggestion);
