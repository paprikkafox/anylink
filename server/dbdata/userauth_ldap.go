...
func (auth AuthLdap) checkData(authData map[string]interface{}) error {
	...
	// Supports domain name and IP, port must be filled in
	if !ValidateIpPort(auth.Addr) && !ValidateDomainPort(auth.Addr) {
		return errors.New("The LDAP server address (including port) is incorrectly filled in")
	}
	if auth.BindName == "" {
	...
	if auth.BaseDn == "" || !ValidateDN(auth.BaseDn) {
		return errors.New("The Base DN of LDAP is filled in incorrectly.")
	}
	if auth.ObjectClass == "" {
		return errors.New("The user class of LDAP is filled in incorrectly")
	}
	if auth.SearchAttr == "" {
		return errors.New("LDAP user unique ID cannot be empty")
	}
	if auth.MemberOf != "" && !ValidateDN(auth.MemberOf) {
		return errors.New("The user group of LDAP is filled in incorrectly")
	}
	return nil
}
...
func (auth AuthLdap) checkUser(name, pwd string, g *Group) error {
	...
	if err != nil {
		return fmt.Errorf("%s %s", name, "LDAP server connection abnormality, please check the server and port")
	}
	defer con.Close()
	// Connect to LDAP
	...
	if err != nil {
		return fmt.Errorf("%s The LDAP administrator DN or password is incorrectly entered %s", name, err.Error())
	}
	if auth.ObjectClass == "" {
